...
namespace NumbersBlast.Opponent
{
    ...
    public sealed class TurnController : MonoBehaviour
    {
        ...
        private IEnumerator MatchLoop()
        {
            while (_active)
            {
                ...
                if (_timedOut && !_moveStarted)
                {
                    ...
                    // pass the turn (board unchanged, so no resolve to wait for). The flash makes the
                    // 5% penalty visible instead of silent.
                    _session.CancelActiveDrag();
                    _session.ApplyTimeoutPenalty(PlayerSide.Local);
                    turnTimer.Stop();
                    turnView.ShowTimeout();
        ...
        private void BeginOpponentTurn()
        {
            ...
            // Per the brief, EVERY turn runs the 20s clock — the opponent's countdown is visible too
            // (orange tint = their clock). The opponent's act always finishes well under the limit, and
            // OnTimeout only ever penalises the local side, so this can never mis-fire a penalty.
